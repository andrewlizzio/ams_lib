...
module AMS::Sketchup
  class << self
    ...
    # Show/hide all trays.
    # @param [Boolean] state
    # @return [Fixnum] Number of trays shown or hidden.
    # @since 3.5.0
    def show_trays(state)
    end

    # Close all trays.
    # @return [Fixnum] Number of trays closed.
    # @since 3.5.0
    def close_trays
    end
    ...
    # @note Any errors occurring in the threaded block will be ignored.
    # @param [String] caption Title
    # @param [String] text Message
    # @param [Fixnum] type Messagebox configuration flags
    # @param [Fixnum] owner_handle Handle to a valid window or nil.
    # @return [Fixnum, nil] A handle to the thread with the messagebox or nil
    #   on failure.
    # @yield Triggered when the messagebox is closed.
    # @yieldparam [Fixnum] result Message result
    # @see https://msdn.microsoft.com/en-us/library/windows/desktop/ms645505(v=vs.85).aspx MessageBox
    # @since 3.5.0
    # @example Process while loop until a messagebox is closed.
    #   caption = 'Decision'
