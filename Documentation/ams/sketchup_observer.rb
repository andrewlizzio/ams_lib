# SketchUpObserver allows monitoring and making decisions to the input events
# and messages reaching the SketchUp window procedure.
# @abstract
# @note SWO events are capable of monitoring window events. SWP, in addition to
#   monitoring, are also capable making decisions to the window events.
#   Returning 1 for an SWP event will prevent the event from reaching the
#   SketchUp window procedure; any other return value won't block the event. If
#   more than one observers install SWP events and one of them returns 1, the
#   event will be blocked, regardless of whether other observers wanted the
#   event to be processed by the SketchUp window procedure.
# @note An observer can be a class, a module, or a class instance. Your
#   observer will work as long as the callback methods are public.
# @note Including all observer methods into your class is not necessary. You may
#   include/elude those as you wish.
...
class AMS::SketchupObserver
  ...
  # Triggered whenever there is an error in any of the observer methods, with
  # exception to this, particular observer method.
  # @note An error will not force the observer to deactivate. If you want the
  #   observer to deactivate on error, then this is the observer method
  #   implement.
  ...
  # @param (see #swp_on_lbutton_down)
  # @return (see #swp_on_lbutton_down)
  # @note Returning 1 doesn't prevent the mouse from moving, but it does prevent
  #   the cursor position change event from being sent to the viewport window.
  # @since 3.6.0
  ...
  # @note Two additional parameters, +tab_index1+ and +tab_index2+, were added
  #   in 3.5.0.
  # @param [Sketchup::Page] page1 Originally selected page.
  # @param [Sketchup::Page] page2 New selected page.
  # @param [Fixnum] tab_index1
  # @param [Fixnum] tab_index2
  # @since 3.2.0
  def swo_on_page_selected(page1, page2, tab_index1, tab_index2)
  end

  # Called when a scenes page is selected.
  # @note Unlike the +swo_on_page_selected+, this callback function allows
  ...
  # @note This event is useful for creating custom scene transitions.
  # @param [Sketchup::Page] page1 Originally selected page.
  # @param [Sketchup::Page] page2 New selected page.
  # @param [Fixnum] tab_index1
  # @param [Fixnum] tab_index2
  # @return (see #swp_on_lbutton_down)
  # @example Activate scene tab without activating the associated page.
  #   class MySWPObserver
